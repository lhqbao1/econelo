...
export async function generateMetadata({
  ...
  const categoryUrl = productSlug
    ? `${BLOG_BASE_URL}/category/${productSlug}`
    : BLOG_BASE_URL;
  ...
  return {
    title,
    description: `Entdecken Sie Ratgeber, Tipps und Technikartikel rund um ${productName} und E-Mobilitaet.`,
    alternates: {
...
export default async function BlogCategoryPage({ params }: PageProps) {
  ...
  const schema = {
    ...
    "@type": "CollectionPage",
    name: `${productDisplayName} Blog Kategorie - Econelo`,
    url: categoryUrl,
    mainEntity: {
      "@type": "ItemList",
      itemListElement: categoryBlogs.items.map((post: BlogItem) => ({
        ...
        author: { "@type": "Person", name: "Econelo Redaktion" },
      })),
    },
  };
