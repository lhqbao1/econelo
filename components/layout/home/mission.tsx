...
const stats = [
    { value: 650, label: "Check charge status remotely through Ether app", unit: "m" },
    { value: 564, label: "Check charge status remotely through Ether app", unit: "+" },
    { value: 125, label: "Check charge status remotely through Ether app", unit: "k" },
];
...
const MissionSection = () => {
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            // set initial states
            gsap.set([".left-images", ".right-subheading", ".right-heading", ".right-description"], {
                opacity: 0,
            });
            ...
            <div className="w-11/12 lg:w-8/12 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                <div className="relative flex items-center h-full left-images">
                    ...
                    <div className="absolute left-0 top-0 w-[340px] h-[340px] rounded-tl-2xl overflow-hidden shadow-lg">
                        <Image src="/mission-image-1.jpg" alt="Charging scooter" fill className="object-cover" />
                    </div>

                    {/* Image 2 */}
                    <div className="absolute -bottom-4 -right-2 w-[340px] h-[340px] rounded-2xl overflow-hidden">
                        <Image src="/mission-image-2.jpg" alt="Riding scooter" fill className="object-cover" />
                    </div>
                ...
                <div className="space-y-8">
                    ...
                    <ul className="space-y-2 mt-4">
                        <li className="flex items-center gap-2 text-sm">
                            <Image src={"/check.png"} width={20} height={20} alt="check icon" className="object-contain" />
                            <span>
                                <strong>Detachable battery, take home and charge in 3 hours.</strong>
                            </span>
                        </li>
                        <li className="flex items-center gap-2 text-sm">
                            <Image src={"/check.png"} width={20} height={20} alt="check icon" className="object-contain" />
                            <span>
                                <strong>Enjoy a hassle-free ride by charging just for 3 hours.</strong>
                            </span>
                        </li>
                    </ul>
                ...
                {stats.map((item, index) => (
                    <div key={index} className="flex flex-col items-center justify-center space-y-3">
                        <div className="flex gap-1">
                            ...
                            />
                            {item.unit && (
                                <span className="text-3xl font-bold text-black self-end mb-1">{item.unit}</span>
                            )}
                        </div>
                        <p className="text-gray-500 leading-snug max-w-[240px] text-base">{item.label}</p>
                    </div>
                ))}
            </div>
        </section>
