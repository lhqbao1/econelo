...
import CartTableSkeleton from "./skeleton";
...
const WishlistTable = ({
  ...
  const handleToggleSelect = (item: WishListItem, is_active: boolean) => {
    ...
    updateWishlistItemStatusMutation.mutate({ itemId: item.id, is_active });
  };

  const handleUpdateWishlistItemQuantity = (
    item: WishListItem,
    newQuantity: number,
  ) => {
    if (newQuantity <= 0) {
  ...
  const handleAddToCart = (item: WishListItem) => {
    addItemToWishlistMutation.mutate(
      { itemId: item.id },
      {
    ...
    <div className="col-span-12 md:col-span-8 flex-1">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold mb-6">
          {currentTable ? currentTable : "Shopping Cart"}
        </h2>
      ...
      {/* Footer action bar */}
      {/* <div
        className="sticky bottom-0 lg:mt-12 bg-secondary text-white p-4 rounded-lg"
        style={barStyle}
      >
        <div className="grid grid-cols-12 gap-4 bg-secondary text-white rounded-lg">
          <div className="xl:col-span-7 col-span-12 flex justify-between">
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-3 justify-between">
                <div className="text-white flex gap-1 cursor-pointer p-0 items-center text-sm">
                  <Trash size={20} />
          ...
          <div className="xl:col-span-5 col-span-12 flex gap-6 xl:justify-end justify-center">
            <div className="text-right text-xl">
              <p className="font-semibold">
                {t("total")} ({wishlist?.items.length} {t("items")}){" "}
            ...
            <Button
              className="bg-primary/90 hover:bg-primary cursor-pointer text-white rounded-full px-10 py-2 relative"
              onClick={() => handleAddWishlistToCart(wishlist?.id ?? "")}
            >
